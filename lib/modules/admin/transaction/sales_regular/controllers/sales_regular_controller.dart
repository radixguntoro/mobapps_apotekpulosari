...
class SalesRegularController extends GetxController {
  String id;
  int page = 1;
  int limit = 10;
  String result = '';
  ...
  SalesRegularController({
    this.id,
  });

  void isLoadingTrue() async{
    ModelSalesRegular.isLoading = true;
  }

  void isLoadingFalse() async{
    ModelSalesRegular.isLoading = false;
  }

  void clearData() {
    ModelSalesRegular.barcode = '';
    ModelSalesRegular.total = 0;  
    ModelSalesRegular.grandTotal = 0; 
    ModelSalesRegular.discount = 0;  
    ModelSalesRegular.payment = 0;  
    ModelSalesRegular.balance = 0;
  }
  ...
  void clearPayment() { 
    ...
    ModelSalesRegular.payment = 0;  
    ModelSalesRegular.balance = 0;   
    ModelSalesRegular.grandTotal = ModelSalesRegular.total;
  }

  void clearCart() {
    ModelSalesRegular.getCart = [];
  }
  
  @override

  void onInit() {
    ModelSalesRegular.result = 'ready';
  }

  void scanDataByBarcodeQRCode(barcode) async {
    result = await SalesRegularService.readDataByBarcodeQRCode(barcode);
    ModelSalesRegular.barcode = '';
    ModelSalesRegular.getCart = ModelSalesRegular.getCart;
    if(result == 'notfound') {
      Get.snackbar(
        "Oops!",
  ...
  void deleteCart(id) async {
    ModelSalesRegular.getCart.removeWhere((item) => item['medicines_items_id'] == id);
    calcTotal();
    update();
  }
  ...
  void decrementQtyCart(data) async {
    data['qty'] -= 1;
    if(data['qty'] < 1) {
      ModelSalesRegular.getCart.removeWhere((item) => item['medicines_items_id'] == data['medicines_items_id']);
    }
    calcTotal();
    update();
  }
  ...
  void incrementQtyCart(data) async {
    data['qty'] += 1;
    calcTotal();
    print('Total: ${ModelSalesRegular.total}');
    update();
  }
  ...
  void calcTotal() async {
    double total = 0;
    for(final data in ModelSalesRegular.getCart) {
      total += (double.parse(data['price']) * data['qty']);
    }

    ModelSalesRegular.total = total;
    ModelSalesRegular.grandTotal = total - ModelSalesRegular.discount;
    ModelSalesRegular.balance = ModelSalesRegular.payment > (total - ModelSalesRegular.discount) ? (ModelSalesRegular.payment - (total - ModelSalesRegular.discount)) : 0;
    update();
  }
  ...
  void calcDiscount(val) async {
    if(val.length < 1 || double.parse(val.replaceAll('.','')) > ModelSalesRegular.total) {
      ...
      ModelSalesRegular.discount = double.parse(val.replaceAll('.',''));
    }
    print( ModelSalesRegular.discount);
    calcTotal();
    update();
  }
  ...
  void calcPayment(val) async {
    if(val.length < 1) {
      ...
      ModelSalesRegular.payment = double.parse(val.replaceAll('.',''));
    }
    calcTotal();
    update();
  }
  ...
  void createData() async {
    print(ModelSalesRegular.getCart.toString());
    ModelSalesRegular.getInvoice = {};
    isLoadingTrue();
    update();
    try {
      response = await SalesRegularService.createData();
      id = response['id'];
      if (response['status'] == 1) {
        result = 'success';
        var data = await SalesRegularService.readDataById(id);
        ModelSalesRegular.getInvoice = data;
        clearCart();
        clearPayment();
        readFirst();
        isLoadingFalse();
  ...
  void readFirst() async{
    ...
    page = 1;
    limit = 10;
    ModelSalesRegular.maxData = false;
    var resp = await SalesRegularService.readData(page, limit);
    ModelSalesRegular.getData = resp;
    isLoadingFalse();
    update();
  }

  void readMore() async{
    page = ModelSalesRegular.currentPage + 1;
    var resp = await SalesRegularService.readData(page, limit);
    if (resp.isEmpty) {
      ModelSalesRegular.maxData = true;
    } else {
      ModelSalesRegular.maxData = false;
      ModelSalesRegular.getData += resp;
    }
  ...
  void readDataBySearch(type) async{
    var resp = await SalesRegularService.readDataBySearch(type, page, limit);
    ModelSalesRegular.getData = resp;
    update();
  }
  ...
  void readDataById(id) async {
    ...
    ModelSalesRegular.getInvoice = resp;
    print(ModelSalesRegular.getInvoice);
    update();
  }
  ...
  PdfGrid getGridHeader() {
    ...
    headerRow.cells[2].value = '${ModelSalesRegular.getInvoice['createdAt']}';
    ...
    //Set style
    headerRow1.cells[0].value = '${ModelSalesRegular.getInvoice['id']}';
    ...
    headerRow1.cells[1].value = '${TextTransform.title(ModelSalesRegular.getInvoice['userName'])}';
    ...
    headerRow1.cells[2].value = '${ModelSalesRegular.getInvoice['timeAt']}';
  ...
  PdfGrid getGridBody() {
    ...
    //Add rows
    for (var item in ModelSalesRegular.getInvoice['details']) {
      addProducts(
    ...
    footer1.cells[0].value = '${NumberFormat.currency(locale: 'id', symbol: '', decimalDigits: 0).format(double.parse(ModelSalesRegular.getInvoice['qtyTotal'].toString()))}';
    ...
    footer1.cells[2].value = '${NumberFormat.currency(locale: 'id', symbol: '', decimalDigits: 0).format(double.parse(ModelSalesRegular.getInvoice['total'].toString()))}';
    ...
    footer2.cells[2].value = '${NumberFormat.currency(locale: 'id', symbol: '', decimalDigits: 0).format(double.parse(ModelSalesRegular.getInvoice['discount'].toString()))}';
    ...
    footer3.cells[2].value = '${NumberFormat.currency(locale: 'id', symbol: '', decimalDigits: 0).format(double.parse(ModelSalesRegular.getInvoice['grandTotal'].toString()))}';
    ...
    footer4.cells[2].value = '${NumberFormat.currency(locale: 'id', symbol: '', decimalDigits: 0).format(double.parse(ModelSalesRegular.getInvoice['payment'].toString()))}';
    ...
    footer5.cells[2].value = '${NumberFormat.currency(locale: 'id', symbol: '', decimalDigits: 0).format(double.parse(ModelSalesRegular.getInvoice['balance'].toString()))}';
