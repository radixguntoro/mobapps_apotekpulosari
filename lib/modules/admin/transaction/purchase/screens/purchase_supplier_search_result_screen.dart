...
class PurchaseSupplierSearchResultScreen extends StatelessWidget {
  ...
  PurchaseSupplierSearchResultScreen(
    this.data,
    this.query,
    this.total,
    this.index,
  ...
  Widget build(BuildContext context) {
    return InkWell(
                ...
                children: <Widget>[
                  Container(
                    ...
                    child: RichText(
                      ...
                      text: searchMatch(data['name'], 'name') 
                    ),
                  ),
                  Container(
                    child: Text(
      ...
      onTap: () {
        _purchaseController.setSupplier(data, 'create');
        Get.close(null);
      },
    );
  }
  ...
  TextSpan searchMatch(String match, String state) {
    if (query == null || query == "")
      return TextSpan(
        text: match, 
        style: state == 'supplier' ? negResName : negResId
      );
    ...
    if (refinedMatch.contains(refinedSearch)) {
      if (refinedMatch.substring(0, refinedSearch.length) == refinedSearch) {
        return TextSpan(
          style: state == 'supplier' ? posResName : posResId,
      ...
      return TextSpan(text: match, style: state == 'supplier' ? negResName : negResId);
    }
    return TextSpan(
