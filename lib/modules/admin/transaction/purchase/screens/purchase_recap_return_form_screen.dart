...
import 'package:apotik_pulosari/modules/admin/transaction/purchase/screens/purchase_recap_return_correct_screen.dart';
...
class PurchaseRecapReturnFormScreen extends StatelessWidget {
  final PurchaseController _purchaseController = Get.put(PurchaseController());
  final item;
  final data;

  PurchaseRecapReturnFormScreen(
    this.item,
    this.data,
  );
  @override
  Widget build(BuildContext context) {
    ...
    TextEditingController _valQtyReturn = TextEditingController(text: NumberFormat.currency(locale: 'id', symbol: '', decimalDigits: 0).format(ModelPurchase.qtyReturn));

    return WillPopScope(
      onWillPop: () async {
        Get.off(PurchaseRecapDetailScreen(data), transition: Transition.downToUp, duration: Duration(milliseconds: ModelTransition.speed),);
      ...
      child: Scaffold(
                                    ...
                                    children: [
                                      ...
                                      Container(
                                        child: TextFormField(
                                          ...
                                          onChanged: (value) => {
                                            _purchaseController.calcReturn(item, value, data),

                                            if (value.length < 1 || int.parse(value.replaceAll('.','')) > int.parse(item['qty'].toString())) {
                                              _valQtyReturn = TextEditingController(text: NumberFormat.currency(locale: 'id', symbol: '', decimalDigits: 0).format(ModelPurchase.qtyReturn))
                                            }
                                          },
                                        ),
                                      ),
                                        ...
                                        children: [
                                          GetBuilder(
                                            ...
                                            builder: (resp) {
                                              return Container(
                                                child: Text(
                                                  NumberFormat.currency(locale: 'id', symbol: 'Rp. ', decimalDigits: 2).format((ModelPurchase.totalReturn)),
                                                  style: TextStyle(
        ...
        bottomNavigationBar: Container(
          ...
          child: GetBuilder(
            init: PurchaseController(),
            builder: (resp) {
              return ModelPurchase.qtyReturn > 0 ? _buttonEnable(context) : _buttonDisable(context);
            }
          )
        ),
      ),
    ...
    return Container(
      ...
      child: Container(
        ...
        child: FlatButton(
          ...
          onPressed: () async {
            Get.off(PurchaseRecapReturnCorrectScreen(item, data), transition: Transition.downToUp, duration: Duration(milliseconds: ModelTransition.speed),);
          },
        ),
      ),
    );
