...
import 'package:apotik_pulosari/modules/admin/transaction/purchase/screens/purchase_stock_adjustments_correct_screen.dart';
...
class PurchaseStockAdjustmentsFormScreen extends StatelessWidget {
  final PurchaseController _purchaseController = Get.put(PurchaseController());
  final item;
  final data;

  PurchaseStockAdjustmentsFormScreen(
    this.item,
    this.data,
  );
  @override
  Widget build(BuildContext context) {
    ...
    TextEditingController _valQtyAdjustments = TextEditingController(text: NumberFormat.currency(locale: 'id', symbol: '', decimalDigits: 0).format(ModelPurchase.qtyAdjustments));

    return WillPopScope(
      onWillPop: () async {
        Get.off(PurchaseStockAdjustmentsScreen(data), transition: Transition.downToUp, duration: Duration(milliseconds: ModelTransition.speed),);
        _purchaseController.clearAdjustments();
        return true;
      },
      child: Scaffold(
                                    ...
                                    children: [
                                      ...
                                      Container(
                                        child: TextFormField(
                                          ...
                                          onChanged: (value) => {
                                            _purchaseController.calcAdjustments(item, value, data),

                                            if (value.length < 1 || int.parse(value.replaceAll('.','')) > int.parse(item['qty'].toString())) {
                                              _valQtyAdjustments = TextEditingController(text: NumberFormat.currency(locale: 'id', symbol: '', decimalDigits: 0).format(ModelPurchase.qtyAdjustments))
                                            }
                                          },
                                        ),
                                      ),
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      Container(
                                        child: TextFormField(
                                          style: TextStyle(
                                            color: ModelPurchase.qtyAdjustmentsDifference > 0 ? Colors.black : ColorTheme.danger
                                          ),
                                          textAlign: TextAlign.end,
                                          keyboardType: TextInputType.number,
                                          enabled: false,
                                          controller: TextEditingController(text: NumberFormat.currency(locale: 'id', symbol: '', decimalDigits: 0).format(ModelPurchase.qtyAdjustmentsDifference)),
                                          autofocus: true,
                                          decoration: InputDecoration(
                                            hintText: "0",
                                            contentPadding: EdgeInsets.only(top: 8, bottom: 8, left: 16, right: 16),
        ...
        bottomNavigationBar: Container(
          ...
          child: GetBuilder(
            init: PurchaseController(),
            builder: (resp) {
              return ModelPurchase.qtyAdjustments > 0 ? _buttonEnable(context) : _buttonDisable(context);
            }
          )
        ),
      ),
    ...
    return Container(
      ...
      child: Container(
        ...
        child: FlatButton(
          ...
          onPressed: () async {
            Get.off(PurchaseStockAdjustmentsCorrectScreen(item, data), transition: Transition.downToUp, duration: Duration(milliseconds: ModelTransition.speed),);
          },
        ),
      ),
    );
