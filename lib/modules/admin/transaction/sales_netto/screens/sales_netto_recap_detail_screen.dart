...
import 'package:apotik_pulosari/modules/admin/transaction/sales_netto/screens/sales_netto_recap_item_screen.dart';
...
class SalesNettoRecapDetailScreen extends StatelessWidget {
  final SalesNettoController _salesNettoController = Get.put(SalesNettoController());
  final data;

  SalesNettoRecapDetailScreen(
    this.data
  );
  @override
                      ...
                      child: Row(
                        children: [
                          Expanded(
                            child: Container(
                              child: Text(
                                "Total: ${NumberFormat.currency(locale: 'id', symbol: '', decimalDigits: 0).format((double.parse(data['tr_sales_netto_details'].length.toString())))} barang",
                                style: TextStyle(
                                  fontSize: 12
                                ),
                              ),
                  ...
                  Expanded(
                    child: Container(
                      ...
                      child: ListView.builder(
                        padding: EdgeInsets.all(0),
                        itemCount: data['tr_sales_netto_details'].length > 0 ? data['tr_sales_netto_details'].length : 0,
                        itemBuilder: (context, int index) {
                          return data['tr_sales_netto_details'].length > 0 ? SalesNettoRecapItemScreen(data['tr_sales_netto_details'][index], data, index) : Container();
                        },
                      ),
                    ),
                  ),
      ...
      floatingActionButton: Container(
        ...
        child: FlatButton(
          ...
          onPressed: () async{
            ...
            await _salesNettoController.readDataById(data['tr_sales_transactions_id']);
