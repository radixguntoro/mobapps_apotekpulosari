...
class SalesLabController extends GetxController {
  String id;
  int page = 1;
  int limit = 10;
  String result = '';
  ...
  void isLoadingTrue() async{
    ModelSalesLab.isLoading = true;
  }

  void isLoadingFalse() async{
    ModelSalesLab.isLoading = false;
  }

  void clearData() {
    ModelSalesLab.barcode = '';
    ModelSalesLab.total = 0;  
    ModelSalesLab.grandTotal = 0; 
    ModelSalesLab.discount = 0;  
    ModelSalesLab.payment = 0;  
    ModelSalesLab.balance = 0;
    ModelSalesLab.getCart = [];
  }

  void clearForm() {
    ModelSalesLab.patientName = '';
    ModelSalesLab.patientAge = '';
    ModelSalesLab.patientGlucoseFasting = '';
    ModelSalesLab.patientGlucose2hoursPP = '';
    ModelSalesLab.patientGlucoseRandom = '';
    ModelSalesLab.patientUricAcid = '';
    ModelSalesLab.patientCholesterol = '';
    ModelSalesLab.patientBloodPressure = '';
  }
  ...
  void clearPayment() { 
    ...
    ModelSalesLab.payment = 0;  
    ModelSalesLab.balance = 0;   
    ModelSalesLab.grandTotal = ModelSalesLab.total;
  }
  ...
  void onInit() {
    ModelSalesLab.result = 'ready';
  }

  void scanDataByBarcodeQRCode(barcode) async {
    result = await SalesLabService.readDataByBarcodeQRCode(barcode);
    ModelSalesLab.barcode = '';
    ModelSalesLab.getCart = ModelSalesLab.getCart;
    if(result == 'notfound') {
      Get.snackbar(
        "Oops!",
  ...
  void deleteCart(id) async {
    ModelSalesLab.getCart.removeWhere((item) => item['medicines_items_id'] == id);
    calcTotal();
    update();
  }
  ...
  void decrementQtyCart(data) async {
    data['qty'] -= 1;
    if(data['qty'] < 1) {
      ModelSalesLab.getCart.removeWhere((item) => item['medicines_items_id'] == data['medicines_items_id']);
    }
    calcTotal();
    update();
  }
  ...
  void incrementQtyCart(data) async {
    data['qty'] += 1;
    calcTotal();
    print('Total: ${ModelSalesLab.total}');
    update();
  }
  ...
  void calcTotal() async {
    double total = 0;
    for(final data in ModelSalesLab.getCart) {
      total += (double.parse(data['price']) * data['qty']);
    }

    ModelSalesLab.total = total;
    ModelSalesLab.grandTotal = total - ModelSalesLab.discount;
    ModelSalesLab.balance = ModelSalesLab.payment > (total - ModelSalesLab.discount) ? (ModelSalesLab.payment - (total - ModelSalesLab.discount)) : 0;
    update();
  }

  void calcDiscount(val) async {
    if(val.length < 1 || double.parse(val.replaceAll('.','')) > ModelSalesLab.total) {
      ModelSalesLab.discount = 0;
    } else {
      ModelSalesLab.discount = double.parse(val.replaceAll('.',''));
    }
    calcTotal();
    update();
  }
  ...
  void calcPayment(val) async {
    if(val.length < 1) {
      ...
      ModelSalesLab.payment = double.parse(val.replaceAll('.',''));
    }
    calcTotal();
    update();
  }
  ...
  void createData() async {
    ModelSalesLab.getInvoice = {};
    isLoadingTrue();
    update();
    try {
      response = await SalesLabService.createData();
      id = response['id'];

      if (response['status'] == 1) {
        result = 'success';
        var data = await SalesLabService.readDataById(id);
        ModelSalesLab.getInvoice = data;
        clearCart();
        clearPayment();
        readFirst();
        isLoadingFalse();
  ...
  void readFirst() async{
    ...
    page = 1;
    limit = 10;
    ModelSalesLab.maxData = false;
    var resp = await SalesLabService.readData(page, limit);
    ModelSalesLab.getData = resp;
    isLoadingFalse();
    update();
  }

  void readMore() async{
    page = ModelSalesLab.currentPage + 1;
    var resp = await SalesLabService.readData(page, limit);
    if (resp.isEmpty) {
      ModelSalesLab.maxData = true;
    } else {
      ModelSalesLab.maxData = false;
      ModelSalesLab.getData += resp;
    }
  ...
  void readDataBySearch(type) async{
    var resp = await SalesLabService.readDataBySearch(type, page, limit);
    ModelSalesLab.getData = resp;
    update();
  }
  ...
  void readDataById(id) async {
    ...
    ModelSalesLab.getInvoice = resp;
    print(ModelSalesLab.getInvoice);
    update();
  }
  ...
  PdfGrid getGridHeader() {
    ...
    headerRow.cells[2].value = '${ModelSalesLab.getInvoice['date']}';
    ...
    //Set style
    headerRow1.cells[0].value = '${ModelSalesLab.getInvoice['id']}';
    ...
    headerRow1.cells[1].value = '${TextTransform.title(ModelSalesLab.getInvoice['cashierName'])}';
    ...
    headerRow1.cells[2].value = '${ModelSalesLab.getInvoice['time']}';
  ...
  PdfGrid getGridBody() {
    ...
    //Add rows
    for (var item in ModelSalesLab.getInvoice['details']) {
      addProducts(
    ...
    footer1.cells[0].value = '${NumberFormat.currency(locale: 'id', symbol: '', decimalDigits: 0).format(double.parse(ModelSalesLab.getInvoice['qtyTotal'].toString()))}';
    ...
    footer1.cells[2].value = '${NumberFormat.currency(locale: 'id', symbol: '', decimalDigits: 0).format(double.parse(ModelSalesLab.getInvoice['total'].toString()))}';
    ...
    footer2.cells[2].value = '${NumberFormat.currency(locale: 'id', symbol: '', decimalDigits: 0).format(double.parse(ModelSalesLab.getInvoice['discount'].toString()))}';
    ...
    footer3.cells[2].value = '${NumberFormat.currency(locale: 'id', symbol: '', decimalDigits: 0).format(double.parse(ModelSalesLab.getInvoice['grandTotal'].toString()))}';
    ...
    footer4.cells[2].value = '${NumberFormat.currency(locale: 'id', symbol: '', decimalDigits: 0).format(double.parse(ModelSalesLab.getInvoice['payment'].toString()))}';
    ...
    footer5.cells[2].value = '${NumberFormat.currency(locale: 'id', symbol: '', decimalDigits: 0).format(double.parse(ModelSalesLab.getInvoice['balance'].toString()))}';
