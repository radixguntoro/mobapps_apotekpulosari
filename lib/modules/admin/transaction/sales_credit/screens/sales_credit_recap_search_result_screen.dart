...
import 'package:apotik_pulosari/modules/admin/transaction/sales_credit/screens/sales_credit_recap_detail_screen.dart';
...
class SalesCreditRecapSearchResultScreen extends StatelessWidget {
  final data;
  final query;
  final total;
  final index;

  SalesCreditRecapSearchResultScreen(
    this.data,
    this.query,
    this.total,
    this.index,
      ...
      secondaryActions: data['status'] != 'paid' ? [
        Container(
          ...
          child: FlatButton(
            ...
            onPressed: () async {
              Get.off(SalesCreditRecapRepaymentScreen(data), transition: Transition.downToUp, duration: Duration(milliseconds: ModelTransition.speed),);
            },
          ),
        ),
      ] : [],
                    ...
                    child: Column(
                      ...
                      children: <Widget>[
                        ...
                        Container(
                          ...
                          child: RichText(
                            textScaleFactor: MediaQuery.of(context).textScaleFactor,
                            text: searchMatch((data['customer']['name']).toString(), 'supplier') 
                          ),
                        ),
                        // Container(
                        //   margin: EdgeInsets.only(bottom: 6),
                          ...
                          child: Row(
                            children: [
                              ...
                              Container(
                                ...
                                child: Text(
                                  "${tanggal(DateTime.parse(data['tr_sales']['transaction']['created_at']), shortMonth: true)} ${DateFormat.Hm().format(DateTime.parse(data['tr_sales']['transaction']['created_at']))}",
                                  style: TextStyle(
                                    fontSize: 12,
                                    color: Colors.black54,
